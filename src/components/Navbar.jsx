...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      setLoading(false); // Set user if logged in
    });
  ...
  return (
    <div className="relative">
         <nav
    className="fixed top-0 left-0  flex w-full h-16 border-b border-gray-200/30 shadow-lg backdrop-blur-md  items-center justify-between z-[1] px-3 lg:h-16 lg:px-8">
    <div className="flex gap-3 items-center">
    <button onClick={toggleMenu}><CgMenuGridO className="text-3xl text-[#4F46E5] lg:hidden"/></button>
    {isMenuOpen && <Menu toggleMenu={toggleMenu} isMenuOpen={isMenuOpen}/>}
      <h1 className="lg:text-2xl font-bold text-xl"><span className="text-[#4F46E5]">NAK</span>res.builder</h1>
    </div>
    ...
    {loading ? null : user ? (
        <div className="relative group flex items-center gap-3">
          ...
          <span className=" text-xs lg:text-lg font-semibold cursor-pointer lg:flex hidden">{user?.email || user?.name}</span>
